...
import { industriesDropdownItems, slugify } from "@/shared/menuData";
    ...
    <div className="bg-background text-foreground">
      ...
      <main className="min-h-screen py-16 pt-28 mt-4">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <p className="text-muted-foreground mb-8">Industry-focused solutions built to meet the unique needs of your sector.</p>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {industriesDropdownItems.map((item) => {
              // For Defence, link directly to /defence page
              const isDefence = item.label.toLowerCase() === "defence";
              const linkTo = isDefence ? "/defence" : `/industries/${slugify(item.label)}`;
              return (
                <Link key={item.label} to={linkTo} className="block group">
